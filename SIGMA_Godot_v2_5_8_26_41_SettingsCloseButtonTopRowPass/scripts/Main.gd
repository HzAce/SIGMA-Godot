...
const BUILD_VERSION := "v2.5.8.26.41"
...
var main_menu_fx_layer: Control
...
var radio_station_menu_buttons: Dictionary = {}
...
var collections_equipped_preview_board: BoardView
var collections_equipped_preview_rules: SigmaRules
var collections_equipped_title_label: Label
var collections_equipped_full_set_label: Label
var collections_equipped_pieces_label: Label
var collections_equipped_board_label: Label
var collections_equipped_note_label: Label
var collections_cards_box: VBoxContainer
var collections_view_toggle_button: Button
var collections_grid_view_enabled: bool = true
...
var rules_guide_body_label: RichTextLabel
var rules_guide_snapshot_label: Label
var rules_guide_snapshot_view: Control
var rules_guide_visual_texture: TextureRect
var rules_guide_visual_caption_label: Label
...
var tutorial_replay_confirm_cover: ColorRect
var tutorial_replay_confirm_panel: PanelContainer
var tutorial_replay_confirm_title_label: Label
var tutorial_replay_confirm_body_label: Label
var tutorial_replay_confirm_primary_button: Button
...
var settings_return_context: String = "main_menu"
...
# Global design rule lock — UI text should never require squinting.
# Mobile-first readability wins over decorative small type across the entire game.
const READABILITY_MIN_BUTTON_FONT_SIZE: int = 22
const READABILITY_MIN_SMALL_LABEL_FONT_SIZE: int = 17
const READABILITY_MIN_BODY_FONT_SIZE: int = 22
const READABILITY_MIN_META_FONT_SIZE: int = 19
const READABILITY_MIN_TITLE_FONT_SIZE: int = 32
const READABILITY_MIN_CLOSE_FONT_SIZE: int = 26
const SIGMA_ARROW_ICON_PATH := "res://assets/ui/icons/sigma_arrows/runtime_256/"
const SIGMA_RULE_ICON_PATH := "res://assets/ui/icons/sigma_rules/core/runtime_256/"
const SIGMA_PIECE_INTRO_PATH := "res://assets/ui/tutorial/piece_intro/classic_tokens/runtime_512/gold/"
const MAIN_MENU_BACKGROUND_FX_SCRIPT := preload("res://scripts/MainMenuBackgroundFX.gd")
...
func _build_ui() -> void:
	...
	counts_label.add_theme_font_size_override("font_size", 16)
	...
	board_view.deploy_mode_requested.connect(_on_board_deploy_mode_requested)
	...
	_build_tutorial_replay_confirm_dialog()
	...
	piece_help_label.add_theme_font_size_override("font_size", 16)
	...
	action_log_label.add_theme_font_size_override("font_size", 15)
	...
	mobile_hint_label.add_theme_font_size_override("font_size", 14)
	mobile_hint_label.add_theme_color_override("font_color", Color("#E8EDF2"))
	mobile_hint_label.text = "Tap a piece, double-tap for Deploy range, triple-tap to unselect."
...
func _build_floating_gameplay_hud(parent: Control) -> void:
	...
	hud_silver_panel_label.get_parent().custom_minimum_size = Vector2(150, 44)
	match_row.add_child(hud_silver_panel_label.get_parent())

	var silver_tray: PanelContainer = PanelContainer.new()
	silver_tray.custom_minimum_size = Vector2(132, 44)
	...
	hud_round_chip.get_parent().custom_minimum_size = Vector2(126, 48)
	...
	hud_timer_chip.get_parent().custom_minimum_size = Vector2(116, 48)
	...
	hud_gold_panel_label.get_parent().custom_minimum_size = Vector2(138, 36)
	player_row.add_child(hud_gold_panel_label.get_parent())

	var gold_tray: PanelContainer = PanelContainer.new()
	gold_tray.custom_minimum_size = Vector2(168, 46)
	...
	_apply_global_readability_pass()


func _apply_global_readability_pass(root_node: Node = null) -> void:
	var target_root: Node = self if root_node == null else root_node
	_apply_readability_recursive(target_root)

func _apply_readability_recursive(node: Node) -> void:
	if node is Control:
		_apply_readability_to_control(node)
	for child in node.get_children():
		_apply_readability_recursive(child)

func _apply_readability_to_control(control: Control) -> void:
	# Desktop hover tooltips were creating distracting descriptive popups.
	control.tooltip_text = ""
	var lower_name: String = String(control.name).to_lower()
	if control is Button:
		var button: Button = control
		var button_target: int = READABILITY_MIN_BUTTON_FONT_SIZE
		if button.text == "←":
			button_target = max(button_target, 28)
		elif button.text == "X" or lower_name.find("close") >= 0:
			button_target = max(button_target, READABILITY_MIN_CLOSE_FONT_SIZE)
		_set_minimum_font_size(control, button_target)
		var minimum_size: Vector2 = button.custom_minimum_size
		if minimum_size.y > 0.0 and minimum_size.y < 60.0:
			minimum_size.y = 60.0
		if button.text != "←" and minimum_size.x > 0.0 and minimum_size.x < 168.0:
			minimum_size.x = 168.0
		button.custom_minimum_size = minimum_size
		return
	if control is CheckBox or control is OptionButton:
		_set_minimum_font_size(control, READABILITY_MIN_META_FONT_SIZE)
		return
	if control is Label:
		var label: Label = control
		var label_target: int = READABILITY_MIN_SMALL_LABEL_FONT_SIZE
		if lower_name.find("title") >= 0 or lower_name.find("header") >= 0 or lower_name.find("banner") >= 0:
			label_target = READABILITY_MIN_TITLE_FONT_SIZE
		elif lower_name.find("body") >= 0 or lower_name.find("summary") >= 0 or lower_name.find("feedback") >= 0 or lower_name.find("checklist") >= 0:
			label_target = READABILITY_MIN_BODY_FONT_SIZE
		elif lower_name.find("step") >= 0 or lower_name.find("phase") >= 0 or lower_name.find("progress") >= 0 or lower_name.find("status") >= 0 or lower_name.find("selection") >= 0:
			label_target = READABILITY_MIN_META_FONT_SIZE
		elif lower_name.find("note") >= 0 or lower_name.find("hint") >= 0 or lower_name.find("counts") >= 0 or lower_name.find("prompt") >= 0 or lower_name.find("scenario") >= 0:
			label_target = READABILITY_MIN_SMALL_LABEL_FONT_SIZE
		if label.text.length() >= 100 and label_target < READABILITY_MIN_BODY_FONT_SIZE:
			label_target = READABILITY_MIN_BODY_FONT_SIZE
		_set_minimum_font_size(control, label_target)
		if label.autowrap_mode != TextServer.AUTOWRAP_OFF:
			label.add_theme_constant_override("line_spacing", 4)
		return
	if control is RichTextLabel or control is LineEdit or control is TextEdit:
		_set_minimum_font_size(control, READABILITY_MIN_BODY_FONT_SIZE)

func _set_minimum_font_size(control: Control, target_size: int) -> void:
	if target_size <= 0:
		return
	if control is RichTextLabel:
		var current_rich_size: int = int(control.get_theme_font_size("normal_font_size"))
		if current_rich_size < target_size:
			control.add_theme_font_size_override("normal_font_size", target_size)
			control.add_theme_font_size_override("bold_font_size", target_size)
			control.add_theme_font_size_override("italics_font_size", target_size)
			control.add_theme_font_size_override("bold_italics_font_size", target_size)
			control.add_theme_font_size_override("mono_font_size", target_size)
		return
	var current_size: int = int(control.get_theme_font_size("font_size"))
...
func _make_text_band(text: String, role: String = "section", font_size: int = 18, align: int = HORIZONTAL_ALIGNMENT_CENTER, fill_width: bool = false, autowrap: bool = false) -> Dictionary:
	var bg: Color = Color("#07111E", 0.84)
	var border: Color = Color("#D4AF37", 0.78)
	var font_color: Color = Color("#F1DBA8")
	var radius: int = 18
	...
	match role:
		"hero":
			bg = Color("#050A16", 0.90)
			border = Color("#D4AF37", 0.90)
			font_color = Color("#F4DEA8")
			radius = 26
			border_width = 2
			margin_h = 18
			margin_v = 10
		"caption":
			bg = Color("#091522", 0.82)
			border = Color("#28E0FF", 0.45)
			font_color = Color("#E8D7AF")
			radius = 16
			border_width = 1
			margin_h = 12
			margin_v = 6
		"eyebrow":
			bg = Color("#06111D", 0.80)
			border = Color("#28E0FF", 0.32)
			font_color = Color("#8DD7FF")
			radius = 14
			border_width = 1
			margin_h = 10
			margin_v = 5
	...
	panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL if fill_width else Control.SIZE_SHRINK_CENTER
	panel.add_theme_stylebox_override("panel", _make_rounded_style(bg, border, border_width, radius))
	...
	label.size_flags_horizontal = Control.SIZE_EXPAND_FILL if fill_width else Control.SIZE_SHRINK_CENTER
	label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART if autowrap else TextServer.AUTOWRAP_OFF
	label.add_theme_font_size_override("font_size", font_size)
	label.add_theme_color_override("font_color", font_color)
	label.add_theme_color_override("font_shadow_color", Color("#000000", 0.84))
...
func _build_preview_panel(parent: Control) -> void:
	...
	preview_tags_label.add_theme_font_size_override("font_size", 15)
	...
	preview_body_label.add_theme_font_size_override("font_size", 16)
...
func _build_result_overlay(parent: Control) -> void:
	...
	result_summary_label.add_theme_font_size_override("font_size", 18)
	...
	_apply_global_readability_pass(result_overlay)
...
func _build_tutorial_panel(parent: Control) -> void:
	...
	tutorial_panel.name = "TutorialPanelFullScreenFrame"
	tutorial_panel.set_anchors_preset(Control.PRESET_FULL_RECT)
	tutorial_panel.offset_left = 16
	tutorial_panel.offset_top = 16
	tutorial_panel.offset_right = -16
	tutorial_panel.offset_bottom = -16
	tutorial_panel.add_theme_stylebox_override("panel", _make_rounded_style(Color("#050917", 0.985), Color("#F2C14E"), 2, 28))
	...
	tutorial_panel.add_child(page_margin)
	...
	_add_top_right_close_button(page_box, "Close tutorial and return to Main Menu.", _on_tutorial_close)

	var tutorial_title_bundle: Dictionary = _make_text_band("", "hero", 32, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	var tutorial_title_panel: PanelContainer = tutorial_title_bundle.get("panel") as PanelContainer
	tutorial_title_label = tutorial_title_bundle.get("label") as Label
	tutorial_title_label.name = "TutorialTitleLabel"
	page_box.add_child(tutorial_title_panel)

	var tutorial_step_bundle: Dictionary = _make_text_band("", "section", 19, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	var tutorial_step_panel: PanelContainer = tutorial_step_bundle.get("panel") as PanelContainer
	tutorial_step_label = tutorial_step_bundle.get("label") as Label
	tutorial_step_label.name = "TutorialStepLabel"
	page_box.add_child(tutorial_step_panel)

	var tutorial_progress_bundle: Dictionary = _make_text_band("", "caption", 16, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	var tutorial_progress_panel: PanelContainer = tutorial_progress_bundle.get("panel") as PanelContainer
	tutorial_progress_label = tutorial_progress_bundle.get("label") as Label
	tutorial_progress_label.name = "TutorialProgressLabel"
	page_box.add_child(tutorial_progress_panel)

	var scroll: ScrollContainer = ScrollContainer.new()
	scroll.name = "TutorialFullScreenScroll"
	...
	var tutorial_phase_bundle: Dictionary = _make_text_band("", "caption", 17, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	var tutorial_phase_panel: PanelContainer = tutorial_phase_bundle.get("panel") as PanelContainer
	tutorial_phase_label = tutorial_phase_bundle.get("label") as Label
	tutorial_phase_label.name = "TutorialPhaseLabel"
	content_box.add_child(tutorial_phase_panel)

	tutorial_body_label = RichTextLabel.new()
	tutorial_body_label.name = "TutorialBodyRichText"
	tutorial_body_label.bbcode_enabled = true
	tutorial_body_label.fit_content = true
	tutorial_body_label.scroll_active = false
	...
	tutorial_body_label.custom_minimum_size = Vector2(0, 118)
	tutorial_body_label.add_theme_font_size_override("normal_font_size", 30)
	tutorial_body_label.add_theme_font_size_override("bold_font_size", 30)
	tutorial_body_label.add_theme_font_size_override("italics_font_size", 30)
	tutorial_body_label.add_theme_font_size_override("bold_italics_font_size", 30)
	tutorial_body_label.add_theme_color_override("default_color", Color("#FFFFFF"))
	tutorial_body_label.add_theme_constant_override("line_separation", 10)
	content_box.add_child(tutorial_body_label)

	tutorial_demo_label = Label.new()
	tutorial_demo_label.name = "TutorialDemoLabel"
	...
	tutorial_demo_label.add_theme_font_size_override("font_size", 20)
	tutorial_demo_label.add_theme_color_override("font_color", Color("#F2C14E"))
	content_box.add_child(tutorial_demo_label)

	tutorial_demo_stage = PanelContainer.new()
	tutorial_demo_stage.name = "TutorialVisualStage"
	tutorial_demo_stage.custom_minimum_size = Vector2(0, 330)
	tutorial_demo_stage.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	tutorial_demo_stage.clip_contents = true
	tutorial_demo_stage.add_theme_stylebox_override("panel", _make_rounded_style(Color("#031321", 0.96), Color("#1D3557"), 1, 20))
	content_box.add_child(tutorial_demo_stage)
	tutorial_demo_canvas = Control.new()
	tutorial_demo_canvas.custom_minimum_size = Vector2(0, 330)
	tutorial_demo_canvas.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	tutorial_demo_canvas.size_flags_vertical = Control.SIZE_EXPAND_FILL
	...
	tutorial_checklist_label.name = "TutorialChecklistLabel"
	...
	tutorial_checklist_label.add_theme_font_size_override("font_size", 21)
	...
	content_box.add_child(tutorial_checklist_label)

	tutorial_feedback_label = Label.new()
	tutorial_feedback_label.name = "TutorialFeedbackLabel"
	...
	tutorial_feedback_label.add_theme_font_size_override("font_size", 21)
	tutorial_feedback_label.add_theme_color_override("font_color", Color("#22C55E"))
	content_box.add_child(tutorial_feedback_label)

	var row: HBoxContainer = HBoxContainer.new()
	row.name = "TutorialActionRow"
	row.alignment = BoxContainer.ALIGNMENT_CENTER
	row.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_theme_constant_override("separation", 10)
	...
	_apply_global_readability_pass(tutorial_panel)
...
func _build_settings_panel(parent: Control) -> void:
	...
	page.add_child(title_box)

	var title_bundle: Dictionary = _make_text_band("Settings", "hero", 30, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	title_box.add_child(title_bundle.get("panel") as PanelContainer)

	var note_bundle: Dictionary = _make_text_band("Sound. Radio. Utilities.", "caption", 14, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	title_box.add_child(note_bundle.get("panel") as PanelContainer)
	...
	var audio_header: Dictionary = _make_text_band("Audio", "section", 21, HORIZONTAL_ALIGNMENT_CENTER, true, false)
	audio_box.add_child(audio_header.get("panel") as PanelContainer)
	...
	var radio_header: Dictionary = _make_text_band("SIGMA Radio", "section", 21, HORIZONTAL_ALIGNMENT_CENTER, true, false)
	radio_box.add_child(radio_header.get("panel") as PanelContainer)

	var band_bundle: Dictionary = _make_text_band("SIGMA FM · Player Radio Override", "caption", 12, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	radio_box.add_child(band_bundle.get("panel") as PanelContainer)
...
func _init_tutorial_steps() -> void:
	tutorial_steps = [
		{"title": "Welcome to SIGMA Tutorial", "short": "Welcome", "scenario": "", "kind": SigmaRules.KIND_MONARCH, "intro": true, "body": "Welcome to the SIGMA Tutorial. Learn the pieces, practice live moves, master key rules, and finish with the win condition.", "confirm": "Tutorial started! Mission 1 unlocked.", "next": "Mission 1: Meet the Monarch."},
		...
		{"title": "Mission 2: Hold the Line", "short": "Guardian", "scenario": "", "kind": SigmaRules.KIND_GUARDIAN, "body": "Guardians move 1 space orthogonally (up/down/left/right). They can jump-capture in those same directions, Deploy, Surround, and Elevate.", "confirm": "Guardian ready! Your line is stronger.", "next": "Next mission: meet the Sentinel."},
		{"title": "Mission 3: Watch Every Angle", "short": "Sentinel", "scenario": "", "kind": SigmaRules.KIND_SENTINEL, "body": "Sentinels move and capture 1 space in any direction. They control the spaces around them.", "confirm": "Sentinel online! All directions covered.", "next": "Next mission: meet the Infiltrator."},
		{"title": "Mission 4: Slip Through", "short": "Infiltrator", "scenario": "", "kind": SigmaRules.KIND_INFILTRATOR, "body": "Infiltrators move up to 2 spaces orthogonally (up/down/left/right) through clear spaces. If the first space is clear, they can line up an orthogonal jump-capture.", "confirm": "Infiltrator ready! Lines are open.", "next": "Next mission: meet the Assassin."},
		{"title": "Mission 5: Strike Diagonal", "short": "Assassin", "scenario": "", "kind": SigmaRules.KIND_ASSASSIN, "body": "Assassins move up to 2 spaces diagonally through clear spaces. If the first diagonal space is clear, they can line up a diagonal jump-capture.", "confirm": "Assassin ready! Diagonals are dangerous.", "next": "Next mission: move a Guardian."},
		{"title": "Mission 6: Guardian Drill", "short": "Guardian Move", "scenario": "tutorial_move_guardian", "kind": SigmaRules.KIND_GUARDIAN, "body": "Move your Guardian 1 space orthogonally (up/down/left/right) to the glowing square.", "expected_type": SigmaRules.ACTION_MOVE, "from": Vector2i(4, 4), "to": Vector2i(4, 5), "success": "Nice move! Guardians hold the line.", "next": "Next mission: move the Sentinel."},
		{"title": "Mission 7: Sentinel Drill", "short": "Sentinel Move", "scenario": "tutorial_move_sentinel", "kind": SigmaRules.KIND_SENTINEL, "body": "Move your Sentinel 1 space diagonally to the glowing square. Sentinels can move 1 space in any direction.", "expected_type": SigmaRules.ACTION_MOVE, "from": Vector2i(4, 4), "to": Vector2i(5, 5), "success": "Sentinel online! One space, any direction.", "next": "Next mission: move the Infiltrator."},
		{"title": "Mission 8: Infiltrator Drill", "short": "Infiltrator Move", "scenario": "tutorial_move_infiltrator", "kind": SigmaRules.KIND_INFILTRATOR, "body": "Move your Infiltrator 2 spaces orthogonally (up/down/left/right) through clear spaces.", "expected_type": SigmaRules.ACTION_MOVE, "from": Vector2i(4, 4), "to": Vector2i(4, 6), "success": "Infiltrator ready! Two-space line movement confirmed.", "next": "Next mission: move the Assassin."},
		{"title": "Mission 9: Assassin Drill", "short": "Assassin Move", "scenario": "tutorial_move_assassin", "kind": SigmaRules.KIND_ASSASSIN, "body": "Move your Assassin 2 spaces diagonally through clear spaces to the glowing square.", "expected_type": SigmaRules.ACTION_MOVE, "from": Vector2i(4, 4), "to": Vector2i(2, 6), "success": "Assassin ready! Diagonal pressure confirmed.", "next": "Next mission: move the Monarch."},
		{"title": "Mission 10: Monarch Drill", "short": "Monarch Move", "scenario": "tutorial_move_monarch", "kind": SigmaRules.KIND_MONARCH, "body": "Move your Monarch 1 space to the glowing safe square. Protect it at all costs.", "expected_type": SigmaRules.ACTION_MOVE, "from": Vector2i(4, 4), "to": Vector2i(5, 5), "success": "Clean Monarch step! Stay safe.", "next": "Next mission: jump-capture."},
		{"title": "Mission 11: Jump-Capture!", "short": "Jump", "scenario": "tutorial_jump_capture", "kind": SigmaRules.KIND_GUARDIAN, "body": "Jump-capture over the enemy piece and land on the glowing square.", "expected_type": SigmaRules.ACTION_JUMP, "from": Vector2i(4, 4), "to": Vector2i(2, 4), "success": "CAPTURE! That piece is gone.", "next": "Next mission: Deploy backup."},
		{"title": "Mission 12: Deploy Backup", "short": "Deploy", "scenario": "tutorial_deploy", "kind": SigmaRules.KIND_GUARDIAN, "body": "Turn Deploy ON, tap your piece, then place a Reserve Guardian on the glowing square.", "expected_type": SigmaRules.ACTION_DEPLOY, "from": Vector2i(4, 4), "to": Vector2i(4, 5), "deploy": true, "success": "Backup deployed! Reserve Guardian enters the fight.", "next": "Next mission: create Peril."},
		{"title": "Mission 13: Create Peril", "short": "Peril", "scenario": "tutorial_direct_peril", "kind": SigmaRules.KIND_SENTINEL, "body": "Move your Sentinel so it directly threatens the enemy Monarch.", "expected_type": SigmaRules.ACTION_MOVE, "from": Vector2i(6, 4), "to": Vector2i(5, 4), "success": "PERIL! Monarch under pressure.", "next": "Next mission: escape Peril."},
		{"title": "Mission 14: Escape!", "short": "Escape", "scenario": "tutorial_escape_peril", "kind": SigmaRules.KIND_MONARCH, "body": "Your Monarch is in Peril. Move to the glowing safe square.", "expected_type": SigmaRules.ACTION_MOVE, "from": Vector2i(4, 4), "to": Vector2i(5, 4), "success": "Clean escape! Your Monarch is safe.", "next": "Next mission: force Surrender."},
		{"title": "Mission 15: No Escape", "short": "Surrender", "scenario": "tutorial_surrender", "kind": SigmaRules.KIND_SENTINEL, "body": "Move your Sentinel diagonally to the glowing square. PERIL! If the Monarch has no legal escape, it Surrenders.", "expected_type": SigmaRules.ACTION_MOVE, "from": Vector2i(2, 2), "to": Vector2i(1, 1), "success": "SURRENDER! No escape remains.", "next": "Next mission: trigger Retreat."},
		{"title": "Mission 16: Safe Retreat", "short": "Retreat", "scenario": "tutorial_retreat", "kind": SigmaRules.KIND_SENTINEL, "body": "Box in your own Guardian. Friendly boxed-in pieces Retreat instead of being removed.", "expected_type": SigmaRules.ACTION_MOVE, "from": Vector2i(5, 3), "to": Vector2i(4, 3), "success": "Safe retreat! Guardian returns to reserve.", "next": "Next mission: Elevate."},
		{"title": "Mission 17: Elevate!", "short": "Elevate", "scenario": "tutorial_elevate", "kind": SigmaRules.KIND_GUARDIAN, "body": "Move your Guardian to the enemy back row, then choose what it becomes.", "expected_type": SigmaRules.ACTION_MOVE, "from": Vector2i(1, 4), "to": Vector2i(0, 4), "requires_elevation": true, "success": "Elevate ready! Choose Sentinel, Infiltrator, or Assassin.", "next": "Next mission: final checkpoint."},
...
func _on_board_deploy_mode_requested(value: bool) -> void:
	if not _can_board_accept_input():
		_sync_board_input_lock()
		return
	_clear_preview_state(false)
	_set_deploy_mode(value)
	if value:
		selection_label.text = "Deploy shortcut ON — green spaces are ready."
		_play_sound_cue("confirm")
	else:
		selection_label.text = "Deploy shortcut OFF."
	_update_labels()
...
func _tutorial_has_intro_step() -> bool:
	return not tutorial_steps.is_empty() and bool((tutorial_steps[0] as Dictionary).get("intro", false))

func _tutorial_mission_offset() -> int:
	return 1 if _tutorial_has_intro_step() else 0

func _tutorial_visible_mission_total() -> int:
	return max(1, tutorial_steps.size() - _tutorial_mission_offset())

func _tutorial_visible_mission_number() -> int:
	return clamp(tutorial_index + 1 - _tutorial_mission_offset(), 1, _tutorial_visible_mission_total())

func _tutorial_is_intro_step(step: Dictionary) -> bool:
	return bool(step.get("intro", false))
...
func _on_tutorial_pressed() -> void:
	...
	# Opening Training from the menu should start on the true Welcome page,
	# not on the last card viewed during the current session.
	tutorial_index = 0
...
func _on_tutorial_next() -> void:
	...
	if _tutorial_is_intro_step(step):
		tutorial_completed_steps[tutorial_index] = String(step.get("confirm", "Tutorial started!"))
		tutorial_index = min(tutorial_index + 1, tutorial_steps.size() - 1)
		tutorial_step_loaded = false
		_reset_board_visual_state(true, true)
		selection_label.text = "Begin Tutorial! Mission 1: Meet the Monarch."
		_update_tutorial_panel()
		_animate_tutorial_demo()
		return
	...
	if not is_done:
		if has_practice:
			...
			return
		# Training-card lessons should feel like a page turn, not two copies of the same page.
		# Mark the card complete, then immediately advance to the next mission.
		_play_sound_cue("tutorial_step_complete")
		tutorial_completed_steps[tutorial_index] = String(step.get("confirm", "Mission complete!"))
		tutorial_step_loaded = false
...
func _on_tutorial_replay_step() -> void:
	...
	if String(step.get("scenario", "")) == "":
		selection_label.text = "This is a training card. Use the main tutorial button to continue."
		if tutorial_feedback_label != null:
			tutorial_feedback_label.text = selection_label.text
		return
	_show_tutorial_replay_confirm()
...
func _on_tutorial_back() -> void:
	if tutorial_index > 0:
		...
		# The Welcome page is an entry screen, not a scored/completed mission.
		if _tutorial_is_intro_step(tutorial_steps[tutorial_index] as Dictionary):
			tutorial_completed_steps.erase(tutorial_index)
	_update_tutorial_panel()
	_animate_tutorial_demo()
...
func _update_tutorial_panel() -> void:
	...
	var is_intro: bool = _tutorial_is_intro_step(step)
	tutorial_title_label.text = "Welcome to SIGMA Tutorial" if is_intro else "SIGMA Training"
	var done_mark: String = " ✓" if is_done else ""
	if is_intro:
		tutorial_step_label.text = "Learn the pieces. Practice the rules. Win the board."
	else:
		tutorial_step_label.text = "%s  ·  Mission %d of %d%s" % [String(step.get("title", "Learn SIGMA")), _tutorial_visible_mission_number(), _tutorial_visible_mission_total(), done_mark]
	...
	if tutorial_body_label != null:
		tutorial_body_label.text = _sigma_highlight_rule_text(_tutorial_body_text(step, is_done))
	...
	if tutorial_feedback_label != null:
		if is_intro:
			tutorial_feedback_label.text = "Tap Begin Tutorial when you are ready."
		elif is_finish:
			...
			tutorial_feedback_label.text = "Tap Start Practice to play this live drill."
		else:
			tutorial_feedback_label.text = "Review the token, then tap I Understand."
	if tutorial_load_button != null:
		tutorial_load_button.visible = has_practice and not is_done and not is_intro
		tutorial_load_button.text = "Start Practice" if not tutorial_step_loaded else "Restart Practice"
	if tutorial_replay_button != null:
		tutorial_replay_button.visible = has_practice and is_done and not is_intro
		tutorial_replay_button.disabled = false
		tutorial_replay_button.text = "Replay Practice"
	if tutorial_next_button != null:
		# Practice pages use Start Practice / Replay Practice. Do not show a dead or disabled I Understand button.
		if is_intro:
			tutorial_next_button.visible = true
			tutorial_next_button.disabled = false
			tutorial_next_button.text = "Begin Tutorial"
		elif has_practice and not is_done and not is_finish:
			...
			elif has_practice and is_done and tutorial_index < tutorial_steps.size() - 1:
				tutorial_next_button.text = "Next Mission"
			elif has_practice and is_done:
				tutorial_next_button.text = "Return Home"
			elif not is_done:
...
func _build_radio_station_panel(parent: Control) -> void:
	...
	var title_bundle: Dictionary = _make_text_band("SIGMA Radio", "hero", 30, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	title_box.add_child(title_bundle.get("panel") as PanelContainer)

	var sub_bundle: Dictionary = _make_text_band("Station screen · now playing · playlist pool · menu music", "caption", 14, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	title_box.add_child(sub_bundle.get("panel") as PanelContainer)
	...
	var unlock_note_bundle: Dictionary = _make_text_band("🟢 On-Air = in Radio rotation  ·  🔴 Off-Air = excluded from Radio  ·  🟣 = Main Menu music", "caption", 13, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	page.add_child(unlock_note_bundle.get("panel") as PanelContainer)
	...
	radio_station_menu_buttons.clear()
	for track in AudioManager.get_radio_tracks():
		if typeof(track) != TYPE_DICTIONARY:
			continue
		...
		track_row.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		track_row.add_theme_constant_override("separation", 8)
		track_grid.add_child(track_row)
		var card_button: Button = _add_button(track_row, title_text, "Select or remove this song from your SIGMA Radio pool.", Callable(self, "_on_radio_track_pressed").bind(id))
		...
		var menu_button: Button = _add_button(track_row, "Menu", "Set or clear this song as Main Menu music.", Callable(self, "_on_radio_menu_music_pressed").bind(id))
		menu_button.custom_minimum_size = Vector2(96, 72)
		menu_button.size_flags_horizontal = Control.SIZE_SHRINK_END
		menu_button.add_theme_font_size_override("font_size", 13)
		radio_station_menu_buttons[id] = menu_button
...
func _on_settings_pressed() -> void:
	# v2.5.8.26.40: remember where Settings was opened from. This keeps
	# tutorial practice and paused matches alive when Settings is closed.
	settings_return_context = "main_menu"
	if session_panel != null and session_panel.visible:
		settings_return_context = "pause_menu"
	elif tutorial_active:
		settings_return_context = "tutorial"
	elif rules_guide_panel != null and rules_guide_panel.visible:
		settings_return_context = "rules"
	elif collections_panel != null and collections_panel.visible:
		settings_return_context = "collections"
	elif custom_game_panel != null and custom_game_panel.visible:
		settings_return_context = "custom"
	elif new_game_panel != null and new_game_panel.visible:
		settings_return_context = "new_game"
...
func _radio_menu_music_dot(id: String) -> String:
	return " 🟣" if AudioManager.is_radio_menu_music_track(id) else ""

func _radio_song_status_prefix(id: String, enabled: bool) -> String:
	return "%s%s" % [_radio_air_status_dot(enabled), _radio_menu_music_dot(id)]

func _on_radio_menu_music_pressed(id: String) -> void:
	var assigned: bool = AudioManager.toggle_radio_menu_music_track(id)
	if assigned:
		selection_label.text = "Main Menu music assigned: %s." % AudioManager.get_radio_menu_music_track_title()
	else:
		selection_label.text = "Main Menu music override cleared. Default menu music restored."
	_update_audio_settings_ui()
	_update_radio_settings_ui()
	_play_sound_cue("confirm")
...
func _update_radio_settings_ui() -> void:
	...
	for id in radio_track_buttons.keys():
		...
		button.text = "%s %s · %s\n%s" % [_radio_song_status_prefix(String(id), enabled), "ON AIR" if enabled else "OFF AIR", clean_title, clean_artist]
	for station_id in radio_station_track_buttons.keys():
		...
		station_button.text = "%s %s · %s\n%s" % [_radio_song_status_prefix(String(station_id), station_enabled), "ON AIR" if station_enabled else "OFF AIR", station_title, station_artist]
	for menu_id in radio_station_menu_buttons.keys():
		var menu_button: Button = radio_station_menu_buttons[menu_id]
		if menu_button == null:
			continue
		var is_menu_music: bool = AudioManager.is_radio_menu_music_track(String(menu_id))
		menu_button.text = "🟣 Menu" if is_menu_music else "Set Menu"
	...
	match settings_return_context:
		...
		"rules":
			...
			if rules_guide_panel != null:
				...
				rules_guide_panel.move_to_front()
			_play_sound_cue("page_back")
			_sync_board_input_lock()
		"collections":
			_show_menu_page_backdrop("collections")
			if collections_panel != null:
				collections_panel.visible = true
				collections_panel.move_to_front()
			_play_sound_cue("page_back")
			_sync_board_input_lock()
		"custom":
			...
			if custom_game_panel != null:
				...
				custom_game_panel.move_to_front()
			_play_sound_cue("page_back")
			_sync_board_input_lock()
		"new_game":
			_show_menu_page_backdrop("setup")
...
func _build_main_menu_panel(parent: Control) -> void:
	...
	main_menu_art_rect.texture = load("res://assets/ui/branding/sigma_main_menu_background_clean.png")
	_configure_static_image(main_menu_art_rect, "cover")
	main_menu_art_rect.modulate = Color(1, 1, 1, 1.0)
	main_menu_panel.add_child(main_menu_art_rect)

	main_menu_fx_layer = MAIN_MENU_BACKGROUND_FX_SCRIPT.new()
	main_menu_fx_layer.name = "SigmaMainMenuBackgroundFX"
	main_menu_fx_layer.set_anchors_preset(Control.PRESET_FULL_RECT)
	main_menu_fx_layer.mouse_filter = Control.MOUSE_FILTER_IGNORE
	main_menu_panel.add_child(main_menu_fx_layer)
	...
	tagline.add_theme_color_override("font_color", Color("#F1D49A"))
	tagline.add_theme_color_override("font_shadow_color", Color("#000000", 0.88))
	...
	var preview_caption_bundle: Dictionary = _make_text_band("", "caption", 14, HORIZONTAL_ALIGNMENT_CENTER, false, true)
	var preview_caption_panel: PanelContainer = preview_caption_bundle.get("panel") as PanelContainer
	preview_caption_panel.custom_minimum_size = Vector2(360, 0)
	menu_preview_label = preview_caption_bundle.get("label") as Label
	menu_preview_label.name = "MainMenuPreviewCaptionLabel"
	menu_preview_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	box.add_child(preview_caption_panel)

	continue_game_button = _add_menu_button(box, "Continue", "Jump back into your saved match.", _on_main_menu_continue_game, "continue")
...
func _layout_main_menu_art() -> void:
	...
	if main_menu_fx_layer != null and main_menu_fx_layer.has_method("set_background_rect"):
		main_menu_fx_layer.call("set_background_rect", draw_rect)
...
func _set_menu_preview(preview_id: String) -> void:
	...
	match preview_id:
		"continue":
			...
			label_text = "Continue · resume your saved SIGMA match"
		"full":
...
func _build_new_game_panel(_parent: Control) -> void:
	...
	var title_bundle: Dictionary = _make_text_band("New Game", "hero", 34, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	box.add_child(title_bundle.get("panel") as PanelContainer)

	var subtitle_bundle: Dictionary = _make_text_band("Choose your mode, then preview.", "caption", 15, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	box.add_child(subtitle_bundle.get("panel") as PanelContainer)
	...
	var new_game_preview_title_bundle: Dictionary = _make_text_band("", "section", 22, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	var new_game_preview_title_panel: PanelContainer = new_game_preview_title_bundle.get("panel") as PanelContainer
	new_game_preview_title_label = new_game_preview_title_bundle.get("label") as Label
	new_game_preview_title_label.name = "NewGamePreviewTitleLabel"
	preview_box.add_child(new_game_preview_title_panel)
...
func _build_tutorial_replay_confirm_dialog() -> void:
	tutorial_replay_confirm_cover = ColorRect.new()
	tutorial_replay_confirm_cover.name = "TutorialReplayConfirmCover"
	tutorial_replay_confirm_cover.set_anchors_preset(Control.PRESET_FULL_RECT)
	tutorial_replay_confirm_cover.color = Color(0, 0, 0, 0.68)
	tutorial_replay_confirm_cover.visible = false
	tutorial_replay_confirm_cover.mouse_filter = Control.MOUSE_FILTER_STOP
	add_child(tutorial_replay_confirm_cover)

	tutorial_replay_confirm_panel = _make_center_panel("TutorialReplayConfirmPanel", Vector2(470, 248))
	tutorial_replay_confirm_panel.visible = false
	tutorial_replay_confirm_panel.mouse_filter = Control.MOUSE_FILTER_STOP
	add_child(tutorial_replay_confirm_panel)

	var box: VBoxContainer = VBoxContainer.new()
	box.add_theme_constant_override("separation", 12)
	tutorial_replay_confirm_panel.add_child(box)
	...
	tutorial_replay_confirm_title_label = Label.new()
	tutorial_replay_confirm_title_label.text = "Replay Practice?"
	tutorial_replay_confirm_title_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	tutorial_replay_confirm_title_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	tutorial_replay_confirm_title_label.add_theme_font_size_override("font_size", 25)
	tutorial_replay_confirm_title_label.add_theme_color_override("font_color", Color("#F2C14E"))
	header.add_child(tutorial_replay_confirm_title_label)
	...
	close_btn.pressed.connect(func() -> void:
		_play_sound_cue("button_tap")
		_hide_tutorial_replay_confirm()
	)
	header.add_child(close_btn)

	tutorial_replay_confirm_body_label = Label.new()
	tutorial_replay_confirm_body_label.text = "Replay this practice from the beginning?"
	tutorial_replay_confirm_body_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	tutorial_replay_confirm_body_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	tutorial_replay_confirm_body_label.add_theme_font_size_override("font_size", 18)
	tutorial_replay_confirm_body_label.add_theme_color_override("font_color", Color("#F7F3E8"))
	tutorial_replay_confirm_body_label.custom_minimum_size = Vector2(0, 64)
	box.add_child(tutorial_replay_confirm_body_label)

	var note: Label = Label.new()
	note.text = "Your current attempt will reset, but completed progress stays saved."
	note.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	note.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	note.add_theme_font_size_override("font_size", 13)
	note.add_theme_color_override("font_color", Color("#B8C4D8"))
	...
	_add_button(button_row, "Cancel", "Keep this mission as-is.", _hide_tutorial_replay_confirm)
	tutorial_replay_confirm_primary_button = _add_button(button_row, "Replay Practice", "Restart this tutorial practice.", _on_tutorial_replay_confirmed)
	tutorial_replay_confirm_primary_button.custom_minimum_size = Vector2(190, 58)

func _show_tutorial_replay_confirm() -> void:
	if tutorial_replay_confirm_panel == null:
		_on_tutorial_replay_confirmed()
		return
	var mission_title: String = "this mission"
	if not tutorial_steps.is_empty():
		mission_title = String((tutorial_steps[tutorial_index] as Dictionary).get("title", "this mission"))
	if tutorial_replay_confirm_title_label != null:
		tutorial_replay_confirm_title_label.text = "Replay Practice?"
	if tutorial_replay_confirm_body_label != null:
		tutorial_replay_confirm_body_label.text = "Replay practice for %s?" % mission_title
	if tutorial_replay_confirm_cover != null:
		tutorial_replay_confirm_cover.visible = true
		tutorial_replay_confirm_cover.move_to_front()
	if tutorial_replay_confirm_panel != null:
		tutorial_replay_confirm_panel.offset_left = -tutorial_replay_confirm_panel.custom_minimum_size.x * 0.5
		tutorial_replay_confirm_panel.offset_right = tutorial_replay_confirm_panel.custom_minimum_size.x * 0.5
		tutorial_replay_confirm_panel.offset_top = -tutorial_replay_confirm_panel.custom_minimum_size.y * 0.5
		tutorial_replay_confirm_panel.offset_bottom = tutorial_replay_confirm_panel.custom_minimum_size.y * 0.5
		tutorial_replay_confirm_panel.visible = true
		tutorial_replay_confirm_panel.move_to_front()
	_sync_board_input_lock()
	_play_sound_cue("page_open")

func _hide_tutorial_replay_confirm() -> void:
	if tutorial_replay_confirm_cover != null:
		tutorial_replay_confirm_cover.visible = false
	if tutorial_replay_confirm_panel != null:
		tutorial_replay_confirm_panel.visible = false
...
func _on_tutorial_replay_confirmed() -> void:
	_hide_tutorial_replay_confirm()
	selection_label.text = "Replay Practice! Try it again."
	_on_tutorial_load_step()
...
func _build_custom_game_panel(_parent: Control) -> void:
	...
	_rebuild_custom_game_panel()
	_apply_global_readability_pass(custom_game_panel)
...
func _rebuild_tournament_home_panel() -> void:
	...
	var tournament_title_bundle: Dictionary = _make_text_band("Tournament", "hero", 34, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	custom_content_box.add_child(tournament_title_bundle.get("panel") as PanelContainer)
	var tournament_intro_bundle: Dictionary = _make_text_band("Build a bracket. Play the next match. Keep the action moving.", "caption", 16, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	custom_content_box.add_child(tournament_intro_bundle.get("panel") as PanelContainer)
...
func _rebuild_custom_games_hub_panel() -> void:
	...
	var custom_title_bundle: Dictionary = _make_text_band("Custom Game", "hero", 32, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	custom_content_box.add_child(custom_title_bundle.get("panel") as PanelContainer)
	var custom_intro_bundle: Dictionary = _make_text_band("Build one match your way.", "caption", 15, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	custom_content_box.add_child(custom_intro_bundle.get("panel") as PanelContainer)
...
func _rebuild_custom_single_match_panel() -> void:
	...
	var single_title_bundle: Dictionary = _make_text_band("Single Custom Match", "hero", 32, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	custom_content_box.add_child(single_title_bundle.get("panel") as PanelContainer)
...
func _add_custom_section_label(text: String, parent: Container = null) -> void:
	...
	if target == null:
		return
	var label_bundle: Dictionary = _make_text_band(text, "section", 17, HORIZONTAL_ALIGNMENT_CENTER, true, false)
	target.add_child(label_bundle.get("panel") as PanelContainer)
...
func _tournament_option_tip(options: Array, option_id: String) -> String:
	for item_value in options:
		var item: Dictionary = item_value as Dictionary
		if String(item.get("id", "")) == option_id:
...
func _make_live_description_bar(title_text: String, body_text: String, accent: Color = Color("#00D1FF")) -> PanelContainer:
	var card: PanelContainer = PanelContainer.new()
	card.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	card.custom_minimum_size = Vector2(0, 86)
	card.add_theme_stylebox_override("panel", _make_rounded_style(Color("#061323", 0.96), accent, 2, 20))
	...
	var title: Label = Label.new()
	title.name = "LiveDescriptionTitle"
	...
	body.name = "LiveDescriptionBody"
	body.text = body_text
	body.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	body.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	body.add_theme_font_size_override("font_size", 18)
	body.add_theme_color_override("font_color", Color("#F7F3E8"))
	box.add_child(body)
...
func _rebuild_tournament_builder_panel() -> void:
	...
	var create_title_bundle: Dictionary = _make_text_band("Create Tournament", "hero", 32, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	custom_content_box.add_child(create_title_bundle.get("panel") as PanelContainer)

	var create_summary_bundle: Dictionary = _make_text_band("Pick a format. Add players. Start the bracket.", "caption", 15, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	custom_content_box.add_child(create_summary_bundle.get("panel") as PanelContainer)
	...
	custom_content_box.add_child(_make_live_description_bar(
		_tournament_option_title(_tournament_type_options(), tournament_type_id),
		_tournament_option_tip(_tournament_type_options(), tournament_type_id),
		Color("#00D1FF")
	))
	...
	for item_value in _tournament_type_options():
		...
		var button: Button = Button.new()
		button.text = String(item.get("title", id))
		button.tooltip_text = ""
	...
	custom_content_box.add_child(_make_live_description_bar(
		_tournament_option_title(_tournament_match_mode_options(), tournament_match_mode_id),
		_tournament_option_tip(_tournament_match_mode_options(), tournament_match_mode_id),
		Color("#F2C14E")
	))
	...
	for mode_value in _tournament_match_mode_options():
		...
		mode_button.text = String(mode_item.get("title", mode_id))
		mode_button.tooltip_text = ""
		mode_button.custom_minimum_size = Vector2(0, 62)
...
func _rebuild_tournament_hub_panel() -> void:
	...
	var hub_title_bundle: Dictionary = _make_text_band("Tournament Hub", "hero", 32, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	hub_column.add_child(hub_title_bundle.get("panel") as PanelContainer)
	...
	var note_bundle: Dictionary = _make_text_band("Tournament Hub never auto-starts a match. Choose Next Match, Resume Paused Match, or Edit Setup when you are ready. Tournament saves remain separate from normal Continue Game.", "caption", 14, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	hub_column.add_child(note_bundle.get("panel") as PanelContainer)
...
func _reset_draft_builder() -> void:
	...
	_apply_global_readability_pass(draft_panel)
...
func _init_rules_guide_pages() -> void:
	rules_guide_pages = [
		...
		{"title": "Turn Actions", "snapshot_id": "actions", "body": "On your turn, take one legal action: move 1 piece, jump-capture with 1 piece, or Deploy 1 Reserve Guardian. Captures are optional. Classic play uses Preview, then Confirm. BLITZ! skips Preview for speed."},
		...
		{"title": "Guardian", "snapshot_id": "guardian", "body": "Guardian: moves 1 space orthogonally (up/down/left/right). It can jump-capture in those same directions, Deploy, Surround, and Elevate on the enemy back row."},
		{"title": "Sentinel", "snapshot_id": "sentinel", "body": "Sentinel: moves 1 space in any direction and jump-captures in any direction. It controls the spaces around it."},
		{"title": "Infiltrator", "snapshot_id": "infiltrator", "body": "Infiltrator: moves 1 or 2 spaces orthogonally (up/down/left/right) through clear spaces. If the first space is clear, it can line up an orthogonal jump-capture. No diagonal or L-shaped captures."},
		{"title": "Assassin", "snapshot_id": "assassin", "body": "Assassin: moves 1 or 2 spaces diagonally through clear spaces. If the first diagonal space is clear, it can line up a diagonal jump-capture. No orthogonal or L-shaped captures."},
		{"title": "Deploy", "snapshot_id": "deploy", "body": "Press Deploy, choose a friendly piece, then place 1 Reserve Guardian on an empty adjacent square in a direction that piece can move. Deploy is adjacent only."},
		{"title": "Peril", "snapshot_id": "peril", "body": "A Monarch is in Peril when an enemy piece directly threatens its square by movement geometry. If your Monarch is in Peril, your next action must remove the direct threat or move to safety."},
		...
		{"title": "Advanced Cap", "snapshot_id": "advanced_cap", "body": "Advanced Cap: each player may have at most 3 Sentinels, 3 Infiltrators, and 3 Assassins on the board at one time. Captured or Retreated advanced pieces return to advanced supply and may re-enter only through Elevation."},
...
func _build_collections_panel(_parent: Control) -> void:
	...
	var title_bundle: Dictionary = _make_text_band("Collections Vault", "hero", 28, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	header.add_child(title_bundle.get("panel") as PanelContainer)

	var subtitle_bundle: Dictionary = _make_text_band("Build your SIGMA look. Choose themed Pieces and Boards, then mix them your way.", "caption", 13, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	header.add_child(subtitle_bundle.get("panel") as PanelContainer)

	# v2.5.8.26.38: the top equipped summary was redundant with the live preview card.
	# Keep the label reference hidden for compatibility with older update paths.
	collections_active_label = Label.new()
	collections_active_label.visible = false
	...
	header.add_child(view_row)
	collections_view_toggle_button = _add_button(view_row, "View: Grid", "Switch Collections between Grid view and Stack view.", _on_collections_view_toggle)
	collections_view_toggle_button.custom_minimum_size = Vector2(150, 42)
	...
	_add_collections_equipped_showcase(scroll_box)

	collections_cards_box = VBoxContainer.new()
	collections_cards_box.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	collections_cards_box.add_theme_constant_override("separation", 14)
	scroll_box.add_child(collections_cards_box)
	_rebuild_collections_card_sections()

	collections_status_label = Label.new()
	collections_status_label.visible = false
	scroll_box.add_child(collections_status_label)

	var footer_crest: TextureRect = TextureRect.new()
	footer_crest.name = "CollectionsFooterSigmaCrest"
	footer_crest.texture = load("res://assets/ui/branding/sigma_crest.png")
	footer_crest.custom_minimum_size = Vector2(42, 42)
	footer_crest.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	_configure_static_image(footer_crest, "contain")
	scroll_box.add_child(footer_crest)

	collections_set_active_button = null
...
func _on_collections_view_toggle() -> void:
	collections_grid_view_enabled = not collections_grid_view_enabled
	_rebuild_collections_card_sections()
	_play_sound_cue("button_tap")

func _rebuild_collections_card_sections() -> void:
	if collections_cards_box == null:
		return
	for child_value in collections_cards_box.get_children():
		var child: Node = child_value as Node
		collections_cards_box.remove_child(child)
		child.queue_free()
	if collections_view_toggle_button != null:
		collections_view_toggle_button.text = "View: Grid" if collections_grid_view_enabled else "View: Stack"
		collections_view_toggle_button.tooltip_text = ""
	_add_collection_category_label(collections_cards_box, "Piece Sets")
	var piece_parent: Container = _make_collection_cards_parent(collections_cards_box)
	_add_collection_set_card(piece_parent)
	_add_collection_vector_set_card(piece_parent)
	_add_collection_draconian_set_card(piece_parent)
	_add_collection_lions_den_set_card(piece_parent)
	_add_collection_category_label(collections_cards_box, "Boards")
	var board_parent: Container = _make_collection_cards_parent(collections_cards_box)
	_add_collection_board_card(board_parent)
	_add_collection_vector_board_card(board_parent)
	_add_collection_draconian_board_card(board_parent)
	_add_collection_lions_den_board_card(board_parent)

func _make_collection_cards_parent(parent: Container) -> Container:
	if collections_grid_view_enabled:
		var grid: GridContainer = GridContainer.new()
		grid.columns = 3
		grid.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		grid.add_theme_constant_override("h_separation", 10)
...
func _add_collections_equipped_showcase(parent: Container) -> void:
	var showcase: PanelContainer = PanelContainer.new()
	showcase.name = "CollectionsEquippedBookletHero"
	showcase.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	showcase.custom_minimum_size = Vector2(0, 410)
	showcase.add_theme_stylebox_override("panel", _make_rounded_style(Color("#020817", 0.995), Color("#D4AF37"), 2, 26))
	...
	eyebrow.text = "CURRENTLY EQUIPPED"
	eyebrow.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	eyebrow.add_theme_font_size_override("font_size", 13)
	eyebrow.add_theme_color_override("font_color", Color("#8DD7FF"))
	box.add_child(eyebrow)

	collections_equipped_title_label = Label.new()
	collections_equipped_title_label.text = "Classic SIGMA Set"
	collections_equipped_title_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	collections_equipped_title_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	collections_equipped_title_label.add_theme_font_size_override("font_size", 28)
	collections_equipped_title_label.add_theme_color_override("font_color", Color("#F2C14E"))
	box.add_child(collections_equipped_title_label)

	var preview_shell: PanelContainer = PanelContainer.new()
	preview_shell.name = "EquippedLivePreviewShell"
	preview_shell.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	preview_shell.size_flags_vertical = Control.SIZE_EXPAND_FILL
	preview_shell.custom_minimum_size = Vector2(0, 230)
	preview_shell.clip_contents = true
	preview_shell.add_theme_stylebox_override("panel", _make_rounded_style(Color("#05070B", 0.98), Color("#24435F"), 1, 20))
	box.add_child(preview_shell)
	...
	collections_equipped_preview_board = BoardView.new()
	collections_equipped_preview_board.name = "CollectionsEquippedLiveBoardPreview"
	collections_equipped_preview_board.custom_minimum_size = Vector2(0, 214)
	collections_equipped_preview_board.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	collections_equipped_preview_board.size_flags_vertical = Control.SIZE_EXPAND_FILL
	collections_equipped_preview_board.mouse_filter = Control.MOUSE_FILTER_IGNORE
	collections_equipped_preview_rules = SigmaRules.new()
	collections_equipped_preview_rules.new_game(SigmaRules.classic_config())
	collections_equipped_preview_board.set_rules(collections_equipped_preview_rules)
	collections_equipped_preview_board.set_board_input_enabled(false)
	preview_margin.add_child(collections_equipped_preview_board)

	var detail_panel: PanelContainer = PanelContainer.new()
	detail_panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	detail_panel.add_theme_stylebox_override("panel", _make_rounded_style(Color("#071421", 0.86), Color("#28E0FF", 0.65), 1, 18))
	...
	collections_equipped_full_set_label = Label.new()
	collections_equipped_full_set_label.visible = false
	collections_equipped_full_set_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	collections_equipped_full_set_label.add_theme_font_size_override("font_size", 18)
	collections_equipped_full_set_label.add_theme_color_override("font_color", Color("#F2C14E"))
	detail_box.add_child(collections_equipped_full_set_label)

	collections_equipped_pieces_label = Label.new()
	collections_equipped_pieces_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	collections_equipped_pieces_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	collections_equipped_pieces_label.add_theme_font_size_override("font_size", 15)
	collections_equipped_pieces_label.add_theme_color_override("font_color", Color("#F2C14E"))
	detail_box.add_child(collections_equipped_pieces_label)

	collections_equipped_board_label = Label.new()
	collections_equipped_board_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	collections_equipped_board_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	collections_equipped_board_label.add_theme_font_size_override("font_size", 15)
	collections_equipped_board_label.add_theme_color_override("font_color", Color("#F2C14E"))
	detail_box.add_child(collections_equipped_board_label)

	collections_equipped_note_label = Label.new()
	collections_equipped_note_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	collections_equipped_note_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	collections_equipped_note_label.add_theme_font_size_override("font_size", 13)
	collections_equipped_note_label.add_theme_color_override("font_color", Color("#B8C4D8"))
	detail_box.add_child(collections_equipped_note_label)
	...
	var pieces_button: Button = _add_button(action_row, "Inspect Pieces", "Open the currently equipped piece set.", _open_active_piece_set_detail)
	pieces_button.custom_minimum_size = Vector2(170, 46)
	var board_button: Button = _add_button(action_row, "Inspect Board", "Open the currently equipped board.", _open_active_board_detail)
	board_button.custom_minimum_size = Vector2(170, 46)


func _add_collection_category_label(parent: Container, text: String) -> void:
	var label_bundle: Dictionary = _make_text_band(text, "section", 18, HORIZONTAL_ALIGNMENT_CENTER, true, false)
	parent.add_child(label_bundle.get("panel") as PanelContainer)

func _add_collection_set_card(parent: Container) -> void:
	var set_card: Button = Button.new()
	set_card.name = "ClassicSigmaSetCard"
	set_card.text = ""
	set_card.tooltip_text = ""
	set_card.focus_mode = Control.FOCUS_NONE
	set_card.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	set_card.custom_minimum_size = Vector2(0, 208 if collections_grid_view_enabled else 190)
	...
	_add_collection_card_content(set_card, "Σ", "SET 1", "Classic SIGMA Set", "Unlocked · Equipped", "Default official Piece Set with Classic SIGMA tokens. Pair it with the Classic SIGMA Board or mix it with another Board. Premium casino-chip pieces on the official Classic board theme.", "Tap to inspect pieces and review the matching Board.", Color("#F2C14E"), Color("#06101F"))
...
func _add_collection_board_card(parent: Container) -> void:
	...
	board_card.text = ""
	board_card.tooltip_text = ""
	board_card.focus_mode = Control.FOCUS_NONE
	board_card.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	board_card.custom_minimum_size = Vector2(0, 208 if collections_grid_view_enabled else 190)
...
func _add_collection_vector_set_card(parent: Container) -> void:
	...
	parent.add_child(set_card)
	_add_collection_card_content(set_card, "V", "SET 2", "Obelisk Set", "Unlocked · Piece Set", "Onyx obelisk pieces with neon laser-cut lines. Matching Board: Obelisk Board, an obsidian laser arena with angular frame geometry, glowing channels, and precision sci-fi set identity.", "Tap to inspect Obelisk pieces, review the matching Board, and equip the look.", Color("#00E5FF"), Color("#04111F"))
...
func _add_collection_draconian_set_card(parent: Container) -> void:
	...
	parent.add_child(set_card)
	_add_collection_card_content(set_card, "D", "SET 3", "Dragons Set", "Unlocked · Piece Set", "Dragon-inspired pieces with colorful flames, wings, and glowing eyes. Matching Board: Dragons Board, an obsidian dragon-scale arena with gemmed flame channels and carved wing frame identity.", "Tap to inspect Dragon pieces, review the matching Board, and equip the look.", Color("#FFB000"), Color("#120507"))
...
func _add_collection_lions_den_set_card(parent: Container) -> void:
	...
	parent.add_child(set_card)
	_add_collection_card_content(set_card, "L", "SET 4", "Lion's Den Set", "Unlocked · Piece Set", "Regal lion pieces with colorful flames, elegant royal manes, and glowing colored eyes. Matching Board: Lion's Den Board, a black stone royal arena with golden structure, jewel corners, and mane-inspired energy accents.", "Tap to inspect Lion's Den pieces, review the matching Board, and equip the look.", Color("#D4AF37"), Color("#120806"))
...
func _add_collection_lions_den_board_card(parent: Container) -> void:
	...
	parent.add_child(board_card)
	_add_collection_card_content(board_card, "♔", "BOARD", "Lion's Den Board", "Included in Lion's Den", "Royal lion-themed board with black stone cells, elegant gold frame architecture, lion crests, colorful jewel corners, and subtle mane-flame energy accents.", "Tap to view the Lion's Den Board details and equip the board theme. It changes visuals only, never rules.", Color("#D4AF37"), Color("#0C0504"))

func _add_collection_grid_card_content(card: Button, logo_text: String, tag_text: String, title_text: String, badge_text: String, desc_text: String, hint_text: String, accent: Color, badge_text_color: Color) -> void:
	...
	logo_panel.custom_minimum_size = Vector2(78, 78)
	logo_panel.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	logo_panel.mouse_filter = Control.MOUSE_FILTER_IGNORE
	logo_panel.add_theme_stylebox_override("panel", _make_rounded_style(Color("#020817", 0.96), accent, 2, 39))
	box.add_child(logo_panel)
	...
	mark.add_theme_font_size_override("font_size", 31)
	...
	title.add_theme_color_override("font_color", Color("#F7F3E8"))
	...
	badge.add_theme_stylebox_override("normal", _make_rounded_style(accent, Color("#FFF4B8"), 1, 14))
...
func _add_collection_card_content(card: Button, logo_text: String, tag_text: String, title_text: String, badge_text: String, desc_text: String, hint_text: String, accent: Color, badge_text_color: Color) -> void:
	if collections_grid_view_enabled:
		_add_collection_grid_card_content(card, logo_text, tag_text, title_text, badge_text, desc_text, hint_text, accent, badge_text_color)
		return
	var card_margin: MarginContainer = MarginContainer.new()
	card_margin.set_anchors_preset(Control.PRESET_FULL_RECT)
	card_margin.mouse_filter = Control.MOUSE_FILTER_IGNORE
...
func _build_collections_set_detail_page() -> void:
	...
	var card: PanelContainer = _add_collection_modal_card(collections_set_detail_panel, Vector2(720, 560))
	var box: VBoxContainer = _collection_modal_box(card)
	_add_collection_modal_header(box, "Classic SIGMA Pieces", "Classic SIGMA Pieces · Equipped", _close_collection_set_detail)
	...
	_add_collection_small_note(scroll_box, "Matching Board: Classic SIGMA Board — premium 2.5D control-center board, gold/cyan frame, adaptive retro-premium board music, ambience, Tabletop and Non-tabletop layout foundations, and future 3D board viewer support.")
...
func _build_collections_board_detail_page() -> void:
	...
	var card: PanelContainer = _add_collection_modal_card(collections_board_detail_panel, Vector2(720, 560))
	var box: VBoxContainer = _collection_modal_box(card)
	_add_collection_modal_header(box, "Classic SIGMA Board", "Classic SIGMA Board · Equipped", _close_collection_board_detail)
	_add_collection_info_card(box, "Boards are collectible visual themes. The Classic SIGMA Board is unlocked by default. A Board controls visuals, Tabletop and Non-tabletop layouts, board audio profile, in-game theme music, ambience, and board-specific animation hooks. It does not change SIGMA rules.")
...
func _build_collections_vector_set_detail_page() -> void:
	...
	var card: PanelContainer = _add_collection_modal_card(collections_vector_set_detail_panel, Vector2(720, 585))
	var box: VBoxContainer = _collection_modal_box(card)
	_add_collection_modal_header(box, "Obelisk Pieces", "Obelisk Pieces · Matching Board: Obelisk", _close_collection_vector_set_detail)
	...
	_add_collection_small_note(scroll_box, "Matching Board: Obelisk Board — obsidian/onyx laser grid, angular frame, gold/silver vector lanes, central SIGMA sigil, and sleek future-tactical command center identity.")
	...
	_style_collection_footer_button(equip_button, true)
...
func _build_collections_vector_board_detail_page() -> void:
	...
	var card: PanelContainer = _add_collection_modal_card(collections_vector_board_detail_panel, Vector2(720, 600))
	var box: VBoxContainer = _collection_modal_box(card)
	_add_collection_modal_header(box, "Obelisk Board", "Obelisk Board", _close_collection_vector_board_detail)
	_add_collection_info_card(box, "Obelisk Board is a collectible Board theme: a black onyx/obsidian arena with neon laser grid cuts, angular frame geometry, gold/silver vector lanes, and a central SIGMA sigil. Visuals only — rules do not change.")
	...
	scroll_box.add_theme_constant_override("separation", 12)
	scroll.add_child(scroll_box)
	_add_collection_image_showcase(scroll_box, "res://assets/collections/vector/vector_board_promo.png", Vector2(0, 300), "Obelisk Board · Official SIGMA board preview")
...
func _build_collections_draconian_set_detail_page() -> void:
	...
	var card: PanelContainer = _add_collection_modal_card(collections_draconian_set_detail_panel, Vector2(720, 585))
	var box: VBoxContainer = _collection_modal_box(card)
	_add_collection_modal_header(box, "Dragons Pieces", "Dragon Pieces · Matching Board: Dragons", _close_collection_draconian_set_detail)
	...
	_add_collection_small_note(scroll_box, "Matching Board: Dragons Board — obsidian dragon-scale arena, carved wing frame, colored flame channels, gemmed role accents, and premium fantasy set identity.")
...
func _build_collections_draconian_board_detail_page() -> void:
	...
	var card: PanelContainer = _add_collection_modal_card(collections_draconian_board_detail_panel, Vector2(720, 600))
	var box: VBoxContainer = _collection_modal_box(card)
	_add_collection_modal_header(box, "Dragons Board", "Dragons Board", _close_collection_draconian_board_detail)
	var scroll: ScrollContainer = ScrollContainer.new()
	scroll.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	scroll.size_flags_vertical = Control.SIZE_EXPAND_FILL
	scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
	...
	scroll_box.add_theme_constant_override("separation", 12)
	scroll.add_child(scroll_box)
	_add_collection_image_showcase(scroll_box, "res://assets/collections/draconian/draconian_board_promo.png", Vector2(0, 300), "Dragons Board · Official SIGMA board preview")
...
func _build_collections_lions_den_set_detail_page() -> void:
	...
	var card: PanelContainer = _add_collection_modal_card(collections_lions_den_set_detail_panel, Vector2(720, 585))
	var box: VBoxContainer = _collection_modal_box(card)
	_add_collection_modal_header(box, "Lion's Den Pieces", "Lion's Den Pieces · Matching Board: Lion's Den", _close_collection_lions_den_set_detail)
	var scroll: ScrollContainer = ScrollContainer.new()
	scroll.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	scroll.size_flags_vertical = Control.SIZE_EXPAND_FILL
	scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
	...
	_add_collection_small_note(scroll_box, "Matching Board: Lion's Den Board — regal black-stone arena, elegant golden frame, lion crests, colorful jewel corners, and mane-inspired energy accents.")
...
func _build_collections_lions_den_board_detail_page() -> void:
	...
	var card: PanelContainer = _add_collection_modal_card(collections_lions_den_board_detail_panel, Vector2(720, 600))
	var box: VBoxContainer = _collection_modal_box(card)
	_add_collection_modal_header(box, "Lion's Den Board", "Lion's Den Board", _close_collection_lions_den_board_detail)
	var scroll: ScrollContainer = ScrollContainer.new()
	scroll.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	scroll.size_flags_vertical = Control.SIZE_EXPAND_FILL
	scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
	...
	scroll_box.add_theme_constant_override("separation", 12)
	scroll.add_child(scroll_box)
	_add_collection_image_showcase(scroll_box, "res://assets/collections/lions_den/lions_den_board_promo.png", Vector2(0, 300), "Lion's Den Board · Official SIGMA board preview")
...
func _style_collection_footer_button(button: Button, primary: bool = false) -> void:
	if button == null:
		return
	button.custom_minimum_size = Vector2(520, 76) if primary else Vector2(160, 64)
	button.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	button.clip_text = true
	_style_button(button, "blue" if primary else "menu")
...
func _piece_texture_from_base(base_path: String, kind: String, owner: int) -> Texture2D:
	...
	return _sigma_load_texture(path)
...
func _close_collection_detail_frames_after_equip() -> void:
	if collections_showcase_panel != null:
		collections_showcase_panel.visible = false
	if collections_set_detail_panel != null:
		collections_set_detail_panel.visible = false
...
func _open_active_piece_set_detail() -> void:
	match active_piece_set_id:
		PIECE_SET_VECTOR:
			_open_collection_vector_set_detail()
		PIECE_SET_DRACONIAN:
			_open_collection_draconian_set_detail()
		PIECE_SET_LIONS_DEN:
			_open_collection_lions_den_set_detail()
		_:
			_open_collection_set_detail()

func _open_active_board_detail() -> void:
	match active_board_theme_id:
		BOARD_THEME_VECTOR:
			_open_collection_vector_board_detail()
		BOARD_THEME_DRACONIAN:
			_open_collection_draconian_board_detail()
		BOARD_THEME_LIONS_DEN:
			_open_collection_lions_den_board_detail()
		_:
			_open_collection_board_detail()
...
func _on_collections_set_classic_active() -> void:
	...
	_apply_active_piece_set_to_board_views()
	_update_collections_panel()
	_close_collection_detail_frames_after_equip()
	_play_sound_cue("confirm")
...
func _update_collections_panel() -> void:
	...
	if collections_active_label != null:
		collections_active_label.text = "Equipped: %s · %s" % [active_pieces_name, active_board_name]
	if collections_equipped_title_label != null:
		collections_equipped_title_label.text = active_set_name
	if collections_equipped_full_set_label != null:
		collections_equipped_full_set_label.text = ""
		collections_equipped_full_set_label.visible = false
	if collections_equipped_pieces_label != null:
		collections_equipped_pieces_label.text = "Pieces: %s" % active_pieces_name
	if collections_equipped_board_label != null:
		collections_equipped_board_label.text = "Board: %s" % active_board_name
	if collections_equipped_note_label != null:
		collections_equipped_note_label.text = "Live preview shows your equipped Pieces and Board. Cards below let you inspect, equip, and build your SIGMA identity."
	if collections_equipped_preview_board != null:
		collections_equipped_preview_board.set_active_piece_set(active_piece_set_id)
		collections_equipped_preview_board.set_active_board_theme(active_board_theme_id)
		collections_equipped_preview_board.set_board_input_enabled(false)
		collections_equipped_preview_board.queue_redraw()
...
func _build_rules_guide_panel(parent: Control) -> void:
	rules_guide_panel = PanelContainer.new()
	rules_guide_panel.name = "RulesGuidePanelFullScreenFrame"
	rules_guide_panel.set_anchors_preset(Control.PRESET_FULL_RECT)
	rules_guide_panel.offset_left = 16
	rules_guide_panel.offset_top = 16
	rules_guide_panel.offset_right = -16
	rules_guide_panel.offset_bottom = -16
	rules_guide_panel.add_theme_stylebox_override("panel", _make_rounded_style(Color("#050917", 0.985), Color("#D4AF37"), 2, 28))
	rules_guide_panel.visible = false
	rules_guide_panel.mouse_filter = Control.MOUSE_FILTER_STOP
	...
	page_margin.add_theme_constant_override("margin_top", 16)
	page_margin.add_theme_constant_override("margin_bottom", 20)
	rules_guide_panel.add_child(page_margin)
	...
	page_box.add_theme_constant_override("separation", 10)
	page_margin.add_child(page_box)
	_add_top_right_close_button(page_box, "Close rules guide and return to Main Menu.", _on_rules_guide_close)

	var rules_title_bundle: Dictionary = _make_text_band("", "hero", 32, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	var rules_title_panel: PanelContainer = rules_title_bundle.get("panel") as PanelContainer
	rules_guide_title_label = rules_title_bundle.get("label") as Label
	rules_guide_title_label.name = "RulesGuideTitleLabel"
	page_box.add_child(rules_title_panel)

	var rules_step_bundle: Dictionary = _make_text_band("", "caption", 17, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	var rules_step_panel: PanelContainer = rules_step_bundle.get("panel") as PanelContainer
	rules_guide_step_label = rules_step_bundle.get("label") as Label
	rules_guide_step_label.name = "RulesGuideStepLabel"
	page_box.add_child(rules_step_panel)

	var scroll: ScrollContainer = ScrollContainer.new()
	scroll.name = "RulesGuideFullScreenScroll"
	scroll.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	scroll.size_flags_vertical = Control.SIZE_EXPAND_FILL
	scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
	page_box.add_child(scroll)
	...
	var rules_visual_bundle: Dictionary = _make_text_band("", "caption", 17, HORIZONTAL_ALIGNMENT_CENTER, true, true)
	var rules_visual_panel: PanelContainer = rules_visual_bundle.get("panel") as PanelContainer
	rules_visual_panel.visible = false
	rules_guide_visual_caption_label = rules_visual_bundle.get("label") as Label
	rules_guide_visual_caption_label.name = "RulesGuideVisualCaptionLabel"
	rules_guide_visual_caption_label.visible = false
	content_box.add_child(rules_visual_panel)

	rules_guide_snapshot_view = preload("res://scripts/RulesSnapshotView.gd").new()
	rules_guide_snapshot_view.name = "RulesGuideBoardExample"
	rules_guide_snapshot_view.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	rules_guide_snapshot_view.custom_minimum_size = Vector2(0, 330)
	content_box.add_child(rules_guide_snapshot_view)

	rules_guide_body_label = RichTextLabel.new()
	rules_guide_body_label.name = "RulesGuideBodyRichText"
	rules_guide_body_label.bbcode_enabled = true
	rules_guide_body_label.fit_content = true
	rules_guide_body_label.scroll_active = false
	rules_guide_body_label.selection_enabled = false
	rules_guide_body_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	rules_guide_body_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	rules_guide_body_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	rules_guide_body_label.custom_minimum_size = Vector2(0, 138)
	rules_guide_body_label.add_theme_font_size_override("normal_font_size", 31)
	rules_guide_body_label.add_theme_font_size_override("bold_font_size", 31)
	rules_guide_body_label.add_theme_font_size_override("italics_font_size", 31)
	rules_guide_body_label.add_theme_font_size_override("bold_italics_font_size", 31)
	rules_guide_body_label.add_theme_color_override("default_color", Color("#FFFFFF"))
	rules_guide_body_label.add_theme_constant_override("line_separation", 10)
	content_box.add_child(rules_guide_body_label)

	var row: HBoxContainer = HBoxContainer.new()
	row.name = "RulesGuideActionRow"
	row.alignment = BoxContainer.ALIGNMENT_CENTER
	row.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_theme_constant_override("separation", 10)
	page_box.add_child(row)
	...
	_apply_global_readability_pass(rules_guide_panel)
...
func _update_rules_guide_panel() -> void:
	...
	var visual_texture: Texture2D = _rules_visual_texture_for_page(page)
	if rules_guide_visual_texture != null:
		rules_guide_visual_texture.texture = visual_texture
		rules_guide_visual_texture.visible = visual_texture != null
	if rules_guide_visual_caption_label != null:
		var caption: String = _rules_visual_caption_for_page(page)
		var caption_visible: bool = caption != ""
		rules_guide_visual_caption_label.text = caption
		rules_guide_visual_caption_label.visible = caption_visible
		var caption_panel: Control = null
		if rules_guide_visual_caption_label.get_parent() != null and rules_guide_visual_caption_label.get_parent().get_parent() is Control:
			caption_panel = rules_guide_visual_caption_label.get_parent().get_parent() as Control
		if caption_panel != null:
			caption_panel.visible = caption_visible
	rules_guide_body_label.text = _sigma_highlight_rule_text(String(page.get("body", "")))
...
func _sigma_load_texture(path: String) -> Texture2D:
	if ResourceLoader.exists(path):
		var resource_texture: Texture2D = load(path) as Texture2D
		if resource_texture != null:
			return resource_texture
	# Fallback for raw PNG assets that exist in the project tree but have not been imported yet.
...
func _sigma_arrow_texture(direction_id: String) -> Texture2D:
	var file_name: String = ""
	match direction_id:
		"up":
			file_name = "sigma_arrow_up_blue_256.png"
		"down":
			file_name = "sigma_arrow_down_red_256.png"
		"left":
			file_name = "sigma_arrow_left_green_256.png"
		"right":
			file_name = "sigma_arrow_right_purple_256.png"
		"up_left":
			file_name = "sigma_arrow_up_left_amber_256.png"
		"up_right":
			file_name = "sigma_arrow_up_right_amber_256.png"
		"down_left":
			file_name = "sigma_arrow_down_left_amber_256.png"
		"down_right":
			file_name = "sigma_arrow_down_right_amber_256.png"
		_:
			file_name = "sigma_arrow_up_blue_256.png"
	return _sigma_load_texture(SIGMA_ARROW_ICON_PATH + file_name)

func _sigma_rule_icon_texture(icon_id: String) -> Texture2D:
	var file_name: String = ""
	match icon_id:
		"deploy":
			file_name = "icon_rule_deploy.png"
		"capture":
			file_name = "icon_rule_capture.png"
		"surround":
			file_name = "icon_rule_surround.png"
		"peril":
			file_name = "icon_rule_peril.png"
		"surrender":
			file_name = "icon_rule_surrender.png"
		"elevate":
			file_name = "icon_rule_elevate.png"
		_:
			file_name = ""
	if file_name == "":
		return null
	return _sigma_load_texture(SIGMA_RULE_ICON_PATH + file_name)

func _tutorial_piece_intro_texture(kind: String) -> Texture2D:
	var file_name: String = "gold_%s_512.png" % _classic_piece_file_name(kind)
	return _sigma_load_texture(SIGMA_PIECE_INTRO_PATH + file_name)

func _tutorial_arrow_directions_for_kind(kind: String) -> Array:
	match kind:
		SigmaRules.KIND_GUARDIAN:
...
func _tutorial_rule_icon_id_for_step(step: Dictionary) -> String:
	var scenario: String = String(step.get("scenario", ""))
	var short_text: String = String(step.get("short", "")).to_lower()
	if scenario == "tutorial_deploy" or short_text == "deploy":
		return "deploy"
	if scenario == "tutorial_jump_capture" or short_text == "jump":
		return "capture"
	if scenario == "tutorial_direct_peril" or short_text == "peril":
		return "peril"
	if scenario == "tutorial_surrender" or short_text == "surrender":
		return "surrender"
	if scenario == "tutorial_elevate" or short_text == "elevate":
		return "elevate"
	if short_text == "review":
		return "surrender"
	return ""

func _rules_visual_texture_for_page(page: Dictionary) -> Texture2D:
	var snapshot: String = String(page.get("snapshot_id", ""))
	match snapshot:
		"deploy":
			return _sigma_rule_icon_texture("deploy")
		"actions":
			return _sigma_rule_icon_texture("capture")
		"peril":
			return _sigma_rule_icon_texture("peril")
		"surrender":
			return _sigma_rule_icon_texture("surrender")
		"retreat":
			return _sigma_rule_icon_texture("surround")
		"elevate":
			return _sigma_rule_icon_texture("elevate")
		"monarch":
			return _tutorial_piece_intro_texture(SigmaRules.KIND_MONARCH)
		"guardian":
			return _tutorial_piece_intro_texture(SigmaRules.KIND_GUARDIAN)
		"sentinel":
			return _tutorial_piece_intro_texture(SigmaRules.KIND_SENTINEL)
		"infiltrator":
			return _tutorial_piece_intro_texture(SigmaRules.KIND_INFILTRATOR)
		"assassin":
			return _tutorial_piece_intro_texture(SigmaRules.KIND_ASSASSIN)
	return null

func _rules_visual_caption_for_page(page: Dictionary) -> String:
	var snapshot: String = String(page.get("snapshot_id", ""))
	match snapshot:
		"deploy":
			return "Deploy uses the locked SIGMA rule icon, then the board shows the exact placement."
		"actions":
			return "Move, jump-capture, or Deploy: one legal action per player turn."
		"peril":
			return "Peril means the Monarch is directly threatened by piece geometry."
		"surrender":
			return "Surrender happens when Peril has no legal escape. The Monarch is not captured."
		"retreat":
			return "Friendly boxed-in pieces Retreat. Enemy boxed-in pieces are removed."
		"elevate":
			return "A Guardian that survives on the enemy back row may Elevate."
		"monarch", "guardian", "sentinel", "infiltrator", "assassin":
			return "Piece pages use the actual Classic SIGMA token art."
	return ""



func _sigma_highlight_replace(text: String, term: String, color: String, placeholders: Array) -> String:
	if term.is_empty() or text.find(term) < 0:
		return text
	var token: String = "%%SIGMA_HIGHLIGHT_%d%%" % placeholders.size()
	placeholders.append("[color=%s][b]%s[/b][/color]" % [color, term])
	return text.replace(term, token)

func _sigma_highlight_rule_text(raw_text: String) -> String:
	var text: String = raw_text.strip_edges()

	# Plain-language simplification pass. Keep this conservative so official rules stay intact.
	text = text.replace("one space", "1 space")
	text = text.replace("two spaces", "2 spaces")
	if text.find("orthogonally") >= 0 and text.find("orthogonally (up/down/left/right)") < 0:
		text = text.replace("orthogonally", "orthogonally (up/down/left/right)")
	if text.find("orthogonal ") >= 0 and text.find("orthogonal (up/down/left/right)") < 0:
		text = text.replace("orthogonal ", "orthogonal (up/down/left/right) ")

	var placeholders: Array = []
	var terms: Array = [
		{"term": "orthogonally (up/down/left/right)", "color": "#00D1FF"},
		{"term": "orthogonal (up/down/left/right)", "color": "#00D1FF"},
		{"term": "up/down/left/right", "color": "#00D1FF"},
		{"term": "non-Monarch pieces", "color": "#F2C14E"},
		{"term": "non-Monarch piece", "color": "#F2C14E"},
		{"term": "non-Monarch", "color": "#F2C14E"},
		{"term": "Reserve Guardians", "color": "#22C55E"},
		{"term": "Reserve Guardian", "color": "#22C55E"},
		{"term": "Deploy Reserve Guardians", "color": "#22C55E"},
		{"term": "1 or 2 spaces", "color": "#22C55E"},
		{"term": "up to 2 spaces", "color": "#22C55E"},
		{"term": "2 spaces", "color": "#22C55E"},
		{"term": "1 space", "color": "#22C55E"},
		{"term": "any direction", "color": "#00D1FF"},
		{"term": "diagonally", "color": "#C084FC"},
		{"term": "diagonal", "color": "#C084FC"},
		{"term": "jump-captures", "color": "#F97316"},
		{"term": "jump-capture", "color": "#F97316"},
		{"term": "Jump-captures", "color": "#F97316"},
		{"term": "Jump-Capture", "color": "#F97316"},
		{"term": "Jump-capture", "color": "#F97316"},
		{"term": "Captures", "color": "#F97316"},
		{"term": "captures", "color": "#F97316"},
		{"term": "Capture", "color": "#F97316"},
		{"term": "Deploys", "color": "#22C55E"},
		{"term": "Deploy", "color": "#22C55E"},
		{"term": "Peril", "color": "#EF4444"},
		{"term": "PERIL", "color": "#EF4444"},
		{"term": "Surrenders", "color": "#F2C14E"},
		{"term": "Surrendered", "color": "#F2C14E"},
		{"term": "Surrender", "color": "#F2C14E"},
		{"term": "SURRENDER", "color": "#F2C14E"},
		{"term": "Surrounds", "color": "#F97316"},
		{"term": "Surround", "color": "#F97316"},
		{"term": "Elevates", "color": "#C084FC"},
		{"term": "Elevation", "color": "#C084FC"},
		{"term": "Elevate", "color": "#C084FC"},
		{"term": "Overtime", "color": "#F2C14E"},
		{"term": "Retreated", "color": "#00D1FF"},
		{"term": "Retreats", "color": "#00D1FF"},
		{"term": "Retreat", "color": "#00D1FF"},
		{"term": "no legal escape", "color": "#EF4444"},
		{"term": "Monarchs", "color": "#F2C14E"},
		{"term": "Monarch", "color": "#F2C14E"},
		{"term": "Guardians", "color": "#22C55E"},
		{"term": "Guardian", "color": "#22C55E"},
		{"term": "Sentinels", "color": "#00D1FF"},
		{"term": "Sentinel", "color": "#00D1FF"},
		{"term": "Infiltrators", "color": "#22C55E"},
		{"term": "Infiltrator", "color": "#22C55E"},
		{"term": "Assassins", "color": "#C084FC"},
		{"term": "Assassin", "color": "#C084FC"},
		{"term": "Preview", "color": "#00D1FF"},
		{"term": "Confirm", "color": "#22C55E"},
		{"term": "BLITZ!", "color": "#F97316"}
	]

	for item_value in terms:
		var item: Dictionary = item_value as Dictionary
		text = _sigma_highlight_replace(text, String(item.get("term", "")), String(item.get("color", "#F2C14E")), placeholders)

	for i in range(placeholders.size()):
		text = text.replace("%%SIGMA_HIGHLIGHT_%d%%" % i, String(placeholders[i]))
...
func _tutorial_phase_text(step: Dictionary, is_done: bool, has_practice: bool) -> String:
	if _tutorial_is_intro_step(step):
		return "Welcome card · learn the tutorial flow"
	if bool(step.get("finish", false)):
...
func _tutorial_progress_text() -> String:
	if tutorial_steps.is_empty():
		return ""
	var current_step: Dictionary = tutorial_steps[tutorial_index] as Dictionary
	if _tutorial_is_intro_step(current_step):
		return "Welcome"
	var dots: Array = []
	var offset: int = _tutorial_mission_offset()
	for i in range(offset, tutorial_steps.size()):
		if tutorial_completed_steps.has(i):
			...
			dots.append("○")
	return "Step %d/%d   %s" % [_tutorial_visible_mission_number(), _tutorial_visible_mission_total(), " ".join(dots)]
...
func _animate_tutorial_demo() -> void:
	...
	var scenario: String = String(step.get("scenario", ""))
	var short_text: String = String(step.get("short", ""))
	var is_intro: bool = _tutorial_is_intro_step(step)
	...
	box.size_flags_vertical = Control.SIZE_EXPAND_FILL
	box.add_theme_constant_override("separation", 8)
	tutorial_demo_canvas.add_child(box)

	var rule_icon_id: String = _tutorial_rule_icon_id_for_step(step)
	if is_intro:
		var welcome_texture: Texture2D = load("res://assets/ui/branding/sigma_crest.png")
		if welcome_texture != null:
			...
			var welcome_fallback: Label = Label.new()
			welcome_fallback.name = "TutorialWelcomeVisualFallback"
			welcome_fallback.text = "SIGMA"
			welcome_fallback.custom_minimum_size = Vector2(0, 175)
			welcome_fallback.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
			welcome_fallback.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
			welcome_fallback.add_theme_font_size_override("font_size", 74)
			welcome_fallback.add_theme_color_override("font_color", Color("#F2C14E"))
			welcome_fallback.add_theme_color_override("font_outline_color", Color(0, 0, 0, 0.94))
			welcome_fallback.add_theme_constant_override("outline_size", 9)
			box.add_child(welcome_fallback)
	else:
		var visual_texture: Texture2D = null
		if rule_icon_id != "":
			visual_texture = _sigma_rule_icon_texture(rule_icon_id)
		if visual_texture == null:
			visual_texture = _tutorial_piece_intro_texture(kind)
		if visual_texture == null:
			visual_texture = _classic_piece_texture(kind, SigmaRules.OWNER_P1)
		...
		if visual_texture != null:
			...
			var visual_fallback: Label = Label.new()
			visual_fallback.name = "TutorialLargeVisualFallback"
			visual_fallback.text = _tutorial_token_letter(kind)
			visual_fallback.custom_minimum_size = Vector2(0, 175)
			visual_fallback.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
			visual_fallback.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
			visual_fallback.add_theme_font_size_override("font_size", 96)
			visual_fallback.add_theme_color_override("font_color", _tutorial_token_ring_color(kind))
			visual_fallback.add_theme_color_override("font_outline_color", Color(0, 0, 0, 0.92))
			visual_fallback.add_theme_constant_override("outline_size", 8)
			box.add_child(visual_fallback)

	var arrow_dirs: Array = []
	if is_intro:
		arrow_dirs = []
	elif scenario == "" and not bool(step.get("finish", false)):
		arrow_dirs = _tutorial_arrow_directions_for_kind(kind)
	elif scenario == "tutorial_move_guardian":
		arrow_dirs = ["up", "left", "right", "down"]
	elif scenario == "tutorial_escape_peril":
		arrow_dirs = ["right"]

	if not arrow_dirs.is_empty():
		var arrow_row: HBoxContainer = HBoxContainer.new()
		arrow_row.name = "TutorialMovementArrowRow"
		...
		for dir_value in arrow_dirs:
			var arrow_tex: Texture2D = _sigma_arrow_texture(String(dir_value))
			if arrow_tex == null:
				continue
	...
	var name_label: Label = Label.new()
	name_label.name = "TutorialVisualCaption"
	if is_intro:
		name_label.text = "SIGMA Tutorial · Learn pieces, movement, and victory."
	elif rule_icon_id != "":
		name_label.text = String(step.get("short", piece_name)) + " · rule example"
	elif scenario == "" and short_text != "Review":
		name_label.text = "%s · %s" % [piece_name, _tutorial_token_tagline(kind)]
	else:
		name_label.text = "%s · live board mission" % String(step.get("short", piece_name))
	name_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
...
func _update_floating_hud() -> void:
	...
	if hud_mode_chip != null:
		if tutorial_active:
			hud_mode_chip.text = "SIGMA Training"
		elif single_player_enabled:
	...
	if hud_gold_panel_label != null:
		if tutorial_active:
			hud_gold_panel_label.text = "Gold"
		elif single_player_enabled:
	...
	if hud_silver_panel_label != null:
		if tutorial_active:
			hud_silver_panel_label.text = "Silver"
		elif single_player_enabled:
	...
	if hud_hint_chip != null:
		if _is_ai_turn_active() or ai_thinking:
			...
			hud_hint_chip.text = "Your move, %s! Tap a piece. Double-tap Deploy." % rules.get_turn_name()
...
func _update_mobile_hint() -> void:
	if rules.game_over:
		...
		mobile_hint_label.text = "Your move! Blue = move, red = capture, green = Deploy. Double-tap a piece for Deploy range."
