...
func _draw_texture_token_for_set(set_id: String, center: Vector2, radius: float, kind: String, owner: int, lift: float = 0.0, tilt: float = 0.0, moving: bool = false, selected_piece: bool = false) -> bool:
	...
	# v2.5.8.15: keep the piece texture centering from v2.5.8.14, but
	# nudge the animation/pulse center slightly right. On transparent tall pieces,
	# the pulse was reading a little left of the visual piece/tile center.
