...
func _process(_delta: float) -> void:
	# Classic SIGMA Tokens use a subtle idle underglow pulse, so the board
	# redraws continuously. This is presentation-only and never changes legality.
	if token_preview_mode or rules != null or coin_motion_active or living_preview_mode or selected.x >= 0 or deploy_mode or _current_turn_in_peril() or not preview_action.is_empty() or illegal_marker.x >= 0:
		queue_redraw()
...
func _draw_texture_token_for_set(set_id: String, center: Vector2, radius: float, kind: String, owner: int, lift: float = 0.0, tilt: float = 0.0, moving: bool = false, selected_piece: bool = false) -> bool:
	...
	var token_center: Vector2 = center + lift_offset
	var glow_center: Vector2 = center

	var shadow_alpha: float = 0.34 if not moving else 0.24
	draw_circle(glow_center + Vector2(0, radius * (0.34 + lift * 0.10)), radius * (1.10 - lift * 0.07), Color(0, 0, 0, shadow_alpha))
	draw_circle(glow_center + Vector2(0, radius * 0.43), radius * 0.72, Color(0, 0, 0, shadow_alpha * 0.55))
	draw_circle(glow_center, radius * (1.24 + pulse * 0.08), _with_alpha(alternating_glow, 0.13 + pulse * 0.05))
	draw_circle(glow_center, radius * (0.96 + pulse * 0.06), _with_alpha(alternating_glow.lightened(0.12), 0.18 + pulse * 0.08))

	if selected_piece:
		draw_arc(glow_center, radius * (1.23 + pulse * 0.04), 0.0, TAU, 72, _with_alpha(accent.lightened(0.18), 0.78), 4.0)
		draw_arc(glow_center, radius * (1.34 + pulse * 0.05), 0.0, TAU, 72, _with_alpha(owner_glow, 0.30), 3.0)

	if moving:
		draw_arc(glow_center, radius * (1.28 + lift * 0.08), 0.0, TAU, 72, _with_alpha(alternating_glow, 0.34), 3.4)
	...
	draw_arc(token_center, radius * 1.04, -PI * 0.88, PI * 0.08, 40, Color(1, 1, 1, 0.22), 2.8)
	return true
...
func _draw_token_preview() -> void:
	...
	draw_circle(center, radius * (1.46 + preview_pulse * 0.08), _with_alpha(preview_glow, 0.14))
	draw_arc(center, radius * 1.38, 0.0, TAU, 72, _with_alpha(preview_glow, 0.48), 2.0)
...
func _draw_quick_play_menu_animation(pulse: float, tick: float) -> void:
	...
	var center_rect: Rect2 = _cell_rect(Vector2i(4, 4))
	draw_rect(center_rect.grow(-8), _with_alpha(color_gold, 0.08 + pulse * 0.16), true)
	draw_arc(center_rect.get_center(), cell_size * (0.38 + pulse * 0.08), 0.0, TAU, 48, _with_alpha(color_gold, 0.58), 2.5)
	for sparkle_index in range(6):
		var angle: float = float(sparkle_index) / 6.0 * TAU + tick * TAU
		var sparkle: Vector2 = center_rect.get_center() + Vector2(cos(angle), sin(angle)) * cell_size * 1.15
		draw_circle(sparkle, cell_size * 0.035, _with_alpha(color_gold, 0.45 + pulse * 0.35))
...
func _handle_cell_tap(cell: Vector2i) -> void:
	...
	if p != null and int(p.owner) == rules.turn:
		...
		selection_changed.emit(_selection_text(cell, p, legal_actions))
		piece_focus_changed.emit(String(p.kind), int(p.owner), cell)
		queue_redraw()
		return
