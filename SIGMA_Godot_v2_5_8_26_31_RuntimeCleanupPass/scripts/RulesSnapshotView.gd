...
func _board_rect(r: Rect2) -> Rect2:
	...
	var available_w: float = min(r.size.x * 0.56, available_h)
...
func _caption_line_color(line: String) -> Color:
	var lower: String = line.to_lower()
	if lower.find("peril") >= 0 or lower.find("surrender") >= 0 or lower.find("capture") >= 0:
		return Color("#FFB4B4")
	if lower.find("deploy") >= 0 or lower.find("reserve") >= 0 or lower.find("elevate") >= 0:
		return Color("#A7F3D0")
	if lower.find("1 space") >= 0 or lower.find("2 spaces") >= 0 or lower.find("up/down/left/right") >= 0 or lower.find("diagonal") >= 0:
		return Color("#BDEBFF")
	return Color("#E8EDF2")

func _draw_caption(rect: Rect2, headline: String, lines: Array) -> void:
	var font: Font = get_theme_default_font()
	draw_string(font, rect.position + Vector2(0, 34), headline, HORIZONTAL_ALIGNMENT_LEFT, rect.size.x, 27, Color("#F2C14E"))
	var y: float = rect.position.y + 76.0
	for line_value in lines:
		var line_text: String = String(line_value)
		draw_string(font, Vector2(rect.position.x, y), line_text, HORIZONTAL_ALIGNMENT_LEFT, rect.size.x, 20, _caption_line_color(line_text))
		y += 30.0
...
func _draw_action_snapshot(r: Rect2) -> void:
	...
	_draw_caption(_info_rect(r, board), "One player turn", ["Move, Capture, or Deploy.", "Preview, then Confirm.", "BLITZ! is fast play."])
...
func _draw_piece_movement_snapshot(r: Rect2, kind: String, owner_key: String, offsets: Array, headline: String, col: Color) -> void:
	...
	var movement_lines: Array = ["Official Classic token.", "Highlighted spaces show", "movement geometry."]
	match kind:
		"M":
			movement_lines = ["1 space", "any direction.", "Protect the Monarch."]
		"G":
			movement_lines = ["1 space", "orthogonal", "(up/down/left/right)."]
		"S":
			movement_lines = ["1 space", "any direction.", "Jump-captures too."]
		"I":
			movement_lines = ["1 or 2 spaces", "orthogonal", "(up/down/left/right)."]
		"A":
			movement_lines = ["1 or 2 spaces", "diagonal only.", "Can jump-capture."]
	_draw_caption(_info_rect(r, board), headline, movement_lines)
...
func _draw_deploy_snapshot(r: Rect2) -> void:
	...
	_draw_caption(_info_rect(r, board), "Deploy", ["Reserve Guardian enters", "beside a friendly piece", "by legal direction."])
...
func _draw_advanced_cap_snapshot(r: Rect2) -> void:
	...
	_draw_caption(_info_rect(r, board), "Advanced Cap", ["Max 3 Sentinels,", "3 Infiltrators,", "3 Assassins."])
