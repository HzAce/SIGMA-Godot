...
var radio_menu_music_track_id: String = ""
...
func play_menu_music(page_context: String = "main") -> void:
	...
	match _active_menu_context:
		...
		_:
			var menu_music_request: String = _main_menu_music_override_path()
			if menu_music_request.is_empty():
				menu_music_request = MUSIC_MAIN_MENU
			_play_context_music("menu", menu_music_request, -8.0, 0.35)
...
func _current_radio_track_for_continuity() -> Dictionary:
	if _current_music_resolved_path.is_empty():
		return {}
	if _music_player == null or _music_player.stream == null:
		return {}
	if not _music_player.playing and not _music_player.stream_paused:
		return {}
	_load_radio_manifest()
	for track in _radio_tracks:
		if typeof(track) != TYPE_DICTIONARY:
			continue
		var path: String = String((track as Dictionary).get("path", ""))
		if path.is_empty():
			continue
		var resolved_path: String = _resolve_music_path(path)
		if not resolved_path.is_empty() and resolved_path == _current_music_resolved_path:
			return track as Dictionary
	return {}

func _should_keep_current_radio_track() -> bool:
	if not radio_enabled:
		return false
	var current_track: Dictionary = _current_radio_track_for_continuity()
	if current_track.is_empty():
		return false
	var id: String = String(current_track.get("id", ""))
	if id.is_empty():
		return false
	# Off-Air removes a song from active SIGMA Radio rotation. If the current
	# Radio song was just turned Off-Air, the next intentional music refresh can
	# move to another On-Air track. Normal menu/pause/new-game navigation will
	# otherwise keep the active Radio song uninterrupted.
	if _radio_enabled_tracks.has(id) and not bool(_radio_enabled_tracks[id]):
		return false
	if _radio_last_track_id.is_empty():
		_radio_last_track_id = id
	_radio_now_playing_title = String(current_track.get("title", _radio_now_playing_title))
	_radio_now_playing_artist = String(current_track.get("artist", _radio_now_playing_artist))
	return true
...
func _play_context_music(context: String, fallback_path: String, volume_db: float = -8.0, fade_time: float = 0.35, force_next: bool = false) -> void:
	...
	if muted or not music_enabled:
		stop_music(0.0)
		return
	if radio_enabled and not force_next and _should_keep_current_radio_track():
		return
	...
	if radio_enabled and not radio_track.is_empty():
		...
		return
	var menu_override_track: Dictionary = _main_menu_music_override_track()
	if not menu_override_track.is_empty() and String(menu_override_track.get("path", "")) == fallback_path:
		_radio_now_playing_title = String(menu_override_track.get("title", "SIGMA Menu Music"))
		_radio_now_playing_artist = String(menu_override_track.get("artist", "SIGMA Radio"))
	else:
...
func _main_menu_music_override_track() -> Dictionary:
	if radio_menu_music_track_id.is_empty():
		return {}
	var track: Dictionary = _track_by_id(radio_menu_music_track_id)
	if track.is_empty():
		return {}
	var path: String = String(track.get("path", ""))
	if path.is_empty() or _resolve_music_path(path).is_empty():
		return {}
	return track

func _main_menu_music_override_path() -> String:
	var track: Dictionary = _main_menu_music_override_track()
	if track.is_empty():
		return ""
	return String(track.get("path", ""))

func get_radio_menu_music_track_id() -> String:
	_ensure_initialized()
	return radio_menu_music_track_id

func is_radio_menu_music_track(id: String) -> bool:
	_ensure_initialized()
	return not id.is_empty() and id == radio_menu_music_track_id

func set_radio_menu_music_track(id: String) -> bool:
	_ensure_initialized()
	if id.is_empty():
		return clear_radio_menu_music_track()
	var track: Dictionary = _track_by_id(id)
	if track.is_empty():
		return false
	radio_menu_music_track_id = id
	_save_settings()
	force_replay_music_layers()
	return true

func clear_radio_menu_music_track() -> bool:
	_ensure_initialized()
	if radio_menu_music_track_id.is_empty():
		return false
	radio_menu_music_track_id = ""
...
func toggle_radio_menu_music_track(id: String) -> bool:
	_ensure_initialized()
	if is_radio_menu_music_track(id):
		clear_radio_menu_music_track()
		return false
	set_radio_menu_music_track(id)
	return true

func get_radio_menu_music_track_title() -> String:
	_ensure_initialized()
	var track: Dictionary = _track_by_id(radio_menu_music_track_id)
	if track.is_empty():
		return "Default Main Menu Music"
	return String(track.get("title", radio_menu_music_track_id))

func get_radio_menu_music_status_text() -> String:
	_ensure_initialized()
	if radio_menu_music_track_id.is_empty():
		return "Menu Music: Default SIGMA Score"
	var track: Dictionary = _track_by_id(radio_menu_music_track_id)
	if track.is_empty():
		return "Menu Music: Default SIGMA Score"
	return "Menu Music: %s" % String(track.get("title", radio_menu_music_track_id))
...
func get_radio_status_text() -> String:
	...
	return "SIGMA Radio: %s · %s · %s · %s · %s · Now Playing: %s" % [mode, play_mode, selected_text, music_gate, get_radio_menu_music_status_text(), _radio_now_playing_title]
...
func _load_settings() -> void:
	...
	radio_menu_music_track_id = String(config.get_value("radio", "menu_music_track_id", radio_menu_music_track_id))
...
func _save_settings() -> void:
	...
	config.set_value("radio", "menu_music_track_id", radio_menu_music_track_id)
